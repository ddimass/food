...
class Provider extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('phone, inn, nds', 'numerical', 'integerOnly'=>true),
			array('name, contact_person, image', 'length', 'max'=>45),
			array('address', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, phone, address, contact_person, image', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tblInvoices' => array(self::HAS_MANY, 'TblInvoice', 'tbl_provider_id'),
		);
	}
